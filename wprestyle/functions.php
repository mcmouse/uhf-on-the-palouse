<?php
/**
 * wprestyle functions and definitions
 *
 * @package Sections
 */
...
public static function after_setup_theme() {
	 ...
	 * If you're building a theme based on wprestyle, use a find and replace
	 * to change 'wprestyle' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'wprestyle', get_template_directory() . '/languages' );
	...
	add_image_size( 'wprestyle-full-width', 1380, 770, true );
	add_image_size( 'wprestyle-post-standard', 880, 460, true );
	add_image_size( 'wprestyle-project-standard', 880, 460, true );
	add_image_size( 'wprestyle-frontpage', 660, 370, true );

	/**
	 * This theme uses wp_nav_menu() in one location.
	 */
	register_nav_menus( array(
		'primary'   => __( 'Primary Menu', 'wprestyle' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'wprestyle_custom_background_args', array(
	...
	public static function get_related_posts() {
		...
		$posts_per_page = apply_filters( 'wprestyle_related_posts_per_page', 4 );
	...
	public static function get_related_projects() {
		...
		$posts_per_page = apply_filters( 'wprestyle_related_projects_per_page', 4 );
...
function wprestyle_homefeed_excerpt () {
	$theContent = trim(strip_tags(get_the_content()));
		$output = str_replace( '"', '', $theContent);
		$output = str_replace( '\r\n', ' ', $output);
		$output = str_replace( '\n', ' ', $output);
			if (get_theme_mod( 'wprestyle_homefeed_excerpt_length' )) :
			$limit = get_theme_mod( 'wprestyle_homefeed_excerpt_length' );
			else : 
			$limit = '16';
			endif;
